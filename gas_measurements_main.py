...
def choose_digit(input_df, input_col_name, opt_list=[]):
    ...
    if len(opt_list) != 0:
        ...
        if isinstance(input_df[input_col_name].iat[0], int) and input_df[input_col_name].iat[0] >= opt_list[-1] and (input_df[input_col_name].iat[0] - opt_list[-1]) < 2:
        ...
        if opt_list[-1] != 9:
            ...
            filtered_list = filter(lambda digit: abs(digit - opt_list[-1]) <= 1, temp_list)
...
if __name__ == '__main__':
    ...
    results_df = pd.read_excel("Gas_Measurements.xlsx", sheet_name='Results')
    start_value_df = pd.read_excel("Gas_Measurements.xlsx", sheet_name='Setup')
    detailed_results_df = pd.read_excel("Gas_Measurements.xlsx", sheet_name='Detailed_Results')

    results_df = get_results(directory_path, start_value_df, results_df)

    #detailed_results_df = get_detailed_results(directory_path, detailed_results_df)

    # write the new data to the Excel file
    with pd.ExcelWriter('Gas_Measurements.xlsx') as writer:
